...
│   └── ...
├── directus/
│   ├── docker-compose.yaml # Directus CMS
...
База данных PostgreSQL работает как Docker volume и сохраняется в папке `../server/postgres/`. Это означает:
...
# Все изменения автоматически сохраняются в ../server/postgres/
```
...
# База данных автоматически восстановится из postgres/
```
...
tar -czf postgres-backup-$(date +%Y%m%d).tar.gz ../server/postgres/

# Восстановить из резервной копии
tar -xzf postgres-backup-20240101.tar.gz -C ../server/
```
